<%@ page import="grailsapplication2.Comment" %>
...
<div class="fieldcontain ${hasErrors(bean: commentInstance, field: 'user', 'error')} ">
	...
	<g:select id="user" name="user.id" from="${grailsapplication2.User.list()}" optionKey="id" value="${commentInstance?.user?.id}" class="many-to-one" noSelection="['null': '']"/>
...
<div class="fieldcontain ${hasErrors(bean: commentInstance, field: 'feedback', 'error')} required">
	...
	<g:select id="feedback" name="feedback.id" from="${grailsapplication2.Feedback.list()}" optionKey="id" required="" value="${commentInstance?.feedback?.id}" class="many-to-one"/>
